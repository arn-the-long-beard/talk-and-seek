...
import ServerAddress from '../../app/features/Server/ServerAddres'
...
export const checkCookie = (req, next) => {
  ...
  if (cookieToken) {
    ...
    next(setCookieIfNeeded(cookieToken))
  } else {
    next(removeCookieIfNeeded())
  }
}

function setCookieIfNeeded (cookieToken) {
  if (Auth.getToken() === cookieToken) {
    console.log('cookie already registred, no need to save')
  } else {
    console.log('save the cookie')
    var setCookie = cookieTool.serialize('token', cookieToken)
    cookie.setRawCookie(setCookie)
  }
}
function removeCookieIfNeeded () {
  if (Auth.getToken()) {
 ...
 */
export function setInvalidateUser (next) {
  if (Auth.getToken()) {
...
function preparePreload () {
  return {session: { visitor: { login: { didInvalidate: true}}}}
}
